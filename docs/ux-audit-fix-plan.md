...
- [x] **2-1 (H12)** ルート境界の追加: `src/app/error.tsx`(グローバル)、`src/app/not-found.tsx`。日本語コピー+ホームへ戻る導線(動的ルートの不正IDは1-5の `NotFoundState` でカバー済みのため個別 `not-found.tsx` は不要と判断)
- [x] **2-2 (H6)** ログインエラーの分類: Supabaseエラーコード(`email_not_confirmed` / `over_request_rate_limit` / ネットワーク)で分岐し、それぞれ適切な日本語メッセージを表示
- [x] **2-3 (M13, M14)** サインアップ/コールバック改善:
...
- [x] **2-4 (M15)** パスワード欄: 「8文字以上」のヘルパーテキスト+表示/非表示トグルを login/signup 両方に追加(共通 `PasswordInput` コンポーネント新設)
- [x] **2-5 (H8, H9, L12)** OCRフィードバック:
...
- [x] **3-1 (M1)** 日付ナビ: 中央の日付表示をタップ可能にして `<input type="date">` ピッカーを開く+今日以外を表示中は「今日へ」ボタンを表示(`isDefaultValue` は既存)
- [x] **3-2 (M2)** 週ナビ: 計画・レポート両方に「今週へ」ボタン。週ラベルに年を含め(`2026/3/23 - 3/29`)、今週表示中は「今週」バッジ
- [x] **3-3 (M3, L7)** 計画グリッド: 食事ラベル列と日付ヘッダー行を sticky 化。「今日」列のハイライトを視認可能な濃度に変更
- [x] **3-4 (M4, M5, M6)** 破壊的操作の確認: 献立削除・「献立に反映」・ログアウトに AlertDialog を追加。ログアウトは pending 中 disabled + `router.refresh()`
- [x] **3-5 (M20, L10)** 二度押し・重複防止: メニュー選択リストと食品マスタ/レシピ選択行を `isPending` 中 disabled に。追加時に軽いトースト or 行の選択済み表示
- [x] **3-6 (M10, M11)** バリデーション表示:
...
- [x] **3-8 (M12)** 数値初期値: デフォルト `0` をやめ空文字+placeholder に(zod で空→undefined→必須エラー or 0 扱いを明示)。フォーカス時全選択でも可
- [x] **3-9 (H10, M21, L19)** レポート改善:
  - バーをタップで数値表示(選択状態をローカルstateで保持)、または数値を常時表示
  - 週に記録ゼロなら「この週の記録はありません」空状態を表示
  - 全チャートのバーに `aria-label`(「3/24 月曜 1,850kcal」形式)を付与
- [x] **3-10 (M22)** 週次目標比較: 計画が入っている日数×日次目標で比較するか、「7日中N日計画済み」を明記

### Phase 4: PWA・その他改善

- [x] **4-1 (M16)** SW更新フロー: `skipWaiting` を即時実行せず waiting 状態で保持し、`updatefound` 検知で「新しいバージョンがあります」トースト+タップで `SKIP_WAITING` メッセージ→`controllerchange` でリロード
- [x] **4-2 (M17, M18)** キャッシュ戦略修正: 認証必須ルートをプリキャッシュから外し、静的アセット+専用 `/offline` ページのみプリキャッシュ。ナビゲーション失敗時は `/offline` へフォールバック。`addAll` を個別 `add` + 失敗許容に
- [x] **4-3 (L17)** `notificationclick` ハンドラ追加(既存ウィンドウ focus / なければ open)
- [x] **4-4 (M19, L14)** CSVエクスポート: 食事データに期間選択(今月/先月/全期間)を追加。ボタンごとの実行中表示(「エクスポート中…」)。0件時は「データがありません」info トースト
- [x] **4-5 (L16, L18)** トースト位置を `bottom-center`(ボトムナビ上、safe-area 考慮)へ変更し、Toaster をルートレイアウトへ移動。`(main)/loading.tsx` にヘッダープレースホルダを追加
- [x] **4-6 (L1, L2)** 検索改善: セットメニュー一覧・セレクタに検索追加。食品マスタ検索を `name` + `brand` の OR 検索に
- [x] **4-7 (L4, L3)** 一覧の並び順を名前順に変更(または選択可能に)。件数が問題になったら `.range()` ページング導入(現状は保留可)
- [x] **4-8 (L6, L9, L11, L13, L15, L20)** 小改善まとめ:
...
- [x] **4-9 (L5, L8, L21)** 残タスク: レシピ栄養の材料からの自動計算(食品マスタ由来の材料のみ合算+手動上書き可)、計画セルのタップターゲット拡大、認証フォームの不要な `getUser()` 削減
